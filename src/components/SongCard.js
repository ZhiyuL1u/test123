...
export default function SongCard({ songId, handleClose }) {
  ...
  useEffect(() => {
    ...
    //     })
    fetch(`https://${config.server_host}:${config.server_port}/song/${songId}`)
        .then(res => res.json())
        .then(songData => {
          setSongData(songData);
          return fetch(`https://${config.server_host}:${config.server_port}/albums/${songData.album_id}`);
        })
        .then(res => res.json())
        .then(albumData => {
              ...
              ? (
                ...
                <ResponsiveContainer height={250}>
                  ...
                  <RadarChart cx="50%" cy="50%" outerRadius="80%" data={chartData}>
                    ...
                    <PolarRadiusAxis angle={30} domain={[0, 1]} tick={{ fontSize: '12px' }}/>
